...
{
	//int
	protected $id, $edadAnios, $edadMeses, $seHaAutomedicado, $esAlergico, $viveMadre, $vivePadre, $numHermanos, $numHermanosVivos, $numHermanosFinados, $seLeHacenMoretones, $haRequeridoTransfusion, $haTenidoFracturas, $haSidoIntervenido, $haSidoHospitalizado, $exFumador, $exAlcoholico, $exAdicto, $estaBajoTratamiento, $subsecuente, $existe;
	//string
	...
	public function getOdontopediatria()
	{
		return $this->odontopediatria;
	}
	...
	/**
	 * asignar de manera correcta la fecha de nacimiento
	...
	public function setFechaNacimiento($fechaNacimiento)
	{
		...
		if($total > 1) {
			// asignacion normal
			$this->fechaNacimiento = $fecha[2] . '/' . $fecha[1] . '/' . $fecha[0];
			
		} else {
		
			list($valor1, $valor2, $valor3) = explode('/', $fechaNacimiento);
			
			// asignacion normal
			$this->fechaNacimiento = $valor3 . '-' .$valor2 . '-' . $valor1;
			
			if(strlen($valor1) === 4) {
				// comienza por el año
				$this->fechaNacimiento = $valor1 . '-' .$valor2 . '-' . $valor3;
			}
		}
	}
	...
	public function setOdontopediatria(ExpedienteOdontopediatria $expediente)
	{
		$this->odontopediatria = $expediente;
	}
	 ...
	 * @return void
	 */
	public function agregaCita(Cita $cita)
	{
	...
	/**
	 * evaluar si se ha automedicado
	 * @return bool
	 */
	public function haAutomedicado()
	{
		if($this->seHaAutomedicado === 1) {
			return true;
		}
	...
	/**
	 * evaluar si es alergico a algo
	 * @return bool
	 */
	public function esAlergico()
	{
		if($this->esAlergico === 1) {
			return true;
		}
